...
const initLogDir = () => {
  ...
  // ===== log-api =====
  const baseDirLogApi = path.resolve("./log-api");
  const logApiDir = path.join(baseDirLogApi, year, month);

  if (!fs.existsSync(logApiDir)) {
    fs.mkdirSync(logApiDir, { recursive: true });
  }

  // ===== logs =====
  const baseDirLogs = path.resolve("./logs");
  const logsDir = path.join(baseDirLogs, year, month);

...
export const getMonitoringSummaryService = async (c_project) => {
  try {
    ...
    for (const row of rows) {
      ...
      let status =
        monitoring && monitoring.n_status
          ? normalize(monitoring.n_status)
          : "NO_DATA";
      ...
      if (monitoring && monitoring.d_monitoring) {
        ...
        const diffMinutes =
          (now - new Date(monitoring.d_monitoring)) / 1000 / 60;
        ...
        const dataTypesForTerminal = dataTypes.filter(
          (dt) =>
            dt.c_project === row.c_project &&
            dt.c_terminal_type === row.c_terminal_type,
        );
        for (const dataTypeMap of dataTypesForTerminal || []) {
          const dataM = monitoring.data
            ? monitoring.data.find(
                (d) => d.c_data_type === dataTypeMap.c_data_type,
              )
            : null;
          if (dataM) {
    ...
    Object.values(stationMap).forEach((st) => {
      ...
      const hasNoData = st.terminal.some((t) => t.status === "NO_DATA");

      if (hasDanger) {
        const filteredTerminal = st.terminal.filter(
          (t) => t.status === "DANGER",
        );
